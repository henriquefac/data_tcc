...
for diarizacao_target in diarizacao_dirs:
    ...
    output_grafico_plot=output_graph_dir.create_file_path("grafico_plot","png", overwrite=True)
    output_grafico_hist=output_graph_dir.create_file_path("grafico_hist", "png", overwrite=True)
    ...
    # --- GRÁFICO DE LINHAS (TODOS OS FALANTES) ---
    ...
    for speaker in speakers:
        ...
        # evitar erro se o falante tem menos de 2 segmentos
        if len(start) < 2:
            continue
    
        # ajusta listas para pares (n, n+1)
        end_adj = end[:-1]
        start_adj = start[1:]
        ...
        for i, (end_n, start_n_1) in enumerate(zip(end_adj, start_adj)):
            eixo_x.append(end_n)
            eixo_y.append(dif[i])
        ...
        plt.plot(
            ...
            linestyle="-",
            markersize=4,
            label=f"Pausas {speaker}"
        )
    
    plt.title("Distribuição das pausas intra-falante ao longo do áudio (todos os falantes)")
    plt.xlabel("Tempo do áudio (segundos)")
    plt.ylabel("Duração da pausa (segundos)")
    plt.grid(axis="y", linestyle="--", alpha=0.7)
    plt.legend()
    output_grafico = output_graph_dir.create_file_path("output_all_line", "png", overwrite=True)
    plt.savefig(output_grafico_plot, dpi=300, bbox_inches="tight")
    plt.close()
